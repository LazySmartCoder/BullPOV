...
def generate_trade_bill_pdf_before(amount, username, name, tid, email, date, stock, prediction):
    try:
...
def generate_trade_bill_pdf_after(amount, username, name, tid, email, date, stock, prediction, pl, outcome, net):
    try:
        filename = f"assets/Receipts/Trade/{username}-{tid}.pdf"
        c = canvas.Canvas(filename, pagesize=A4)
        width, height = A4
...
def tradeReceipt(request, tid):
    ...
    if not trade.Outcome:
        outcome = "LOST"
    if trade.ActiveStatus:
        generate_trade_bill_pdf_before(trade.Amount, trade.Trader.username, trade.Trader.first_name, tid, trade.Trader.email, str(trade.DateTime)[:10], trade.Stock.Symbol, prediction)
    else:
        if trade.Outcome:
            net = trade.Amount + trade.Return
            returns = trade.Return
        else:
            net = 0.0
            returns = trade.Amount
        generate_trade_bill_pdf_after(trade.Amount, trade.Trader.username, trade.Trader.first_name, tid, trade.Trader.email, str(trade.DateTime)[:10], trade.Stock.Symbol, prediction, returns, outcome, net)
